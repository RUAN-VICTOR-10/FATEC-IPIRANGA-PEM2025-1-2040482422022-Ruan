...
int validar_codigo_produto(char codigo_produto[MAX_PRODUTO]) {
...
// Funcao para validar limites da rua e da gondola
int validar_rua_gondola(int rua, int gondola) {
    return (rua >= 0 && rua < NUM_RUAS && gondola >= 0 && gondola < NUM_GONDOLAS);
}

// Funcao para estocar um produto dentro do limite de 5 lotes por gondola
void estocar_produto(char armazem[NUM_RUAS][NUM_GONDOLAS][MAX_LOTES][MAX_PRODUTO], char codigo_produto[MAX_PRODUTO]) {
...
void retirar_produto(char armazem[NUM_RUAS][NUM_GONDOLAS][MAX_LOTES][MAX_PRODUTO], char codigo_produto[MAX_PRODUTO]) {
    for (int rua = 0; rua < NUM_RUAS; rua++) {
        for (int gondola = 0; gondola < NUM_GONDOLAS; gondola++) {
            for (int i = 0; i < MAX_LOTES; i++) {
